...
工作流默认每天 **北京时间 0:08** 自动执行签到，无需手动操作。
...
默认每天 **北京时间 0:08** 执行，如需修改，编辑 `.github/workflows/checkin.yml`：

```yaml
schedule:
  - cron: '8 16 * * *'  # UTC 16:08 = 北京时间 0:08
```
...
- `8 16 * * *` - 每天 UTC 16:08（北京时间 0:08，当前默认）


### 🔔 飞书通知配置（可选）

签到完成后自动发送通知到飞书会话，包含签到结果、获得额度、Session 失效提醒等信息。
...
4. 在 **权限管理** 中开启：`im:message` 相关权限
5. 发布应用并创建会话，获取 **会话 ID（Chat ID）**
...
| `FEISHU_CHAT_ID` | 接收通知的飞书会话 ID | 是 |

#### 3. 通知内容示例

签到完成后，飞书会话会收到类似以下的消息：

`
📋 NewAPI 签到报告
执行时间: 2026-08-05 00:08:00
──────────────
✅ 成功 (2个)
· 主力站: +500.00K (已签 15 天)
· 备用站: +100.00K (已签 8 天)

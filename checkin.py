...
def load_config_from_local_file(config_path: str = None) -> Optional[str]:
    """Load accounts saved by the local web console."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    candidates = []

    if config_path:
        candidates.append(config_path)
    env_path = os.environ.get('LOCAL_CONFIG_PATH', '')
    if env_path:
        candidates.append(env_path)

    candidates.extend([
        os.path.join(base_dir, 'local_config.json'),
        os.path.join(base_dir, 'newapi_accounts.json'),
    ])
    ...
    for path in candidates:
        ...
        try:
            ...
            if isinstance(accounts, list) and accounts:
                print(f'[local] Loaded {len(accounts)} account(s) from {path}')
                return json.dumps(accounts, ensure_ascii=False)
        except Exception as e:
            print(f'[local] Failed to load {path}: {e}')
...
def main():
    ...
    if not accounts_str:
        accounts_str = load_config_from_local_file() or ''
    ...
    for i, account in enumerate(accounts, 1):
        ...
        elif '签到' in result.get('message', '') and ('今' in result.get('message', '') or 'already' in result.get('message','').lower() or 'repeat' in result.get('message','').lower()):
            success_count += 1  # 今日/今天已签到也视为成功
            print(f'  结果: [成功] {result["message"]}')
            account_result = {
                'name': name,
                'success': True,
